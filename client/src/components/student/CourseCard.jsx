  ...
  return (
    <Link to={'/course/' + course._id} onClick={() => scrollTo(0, 0)} className='border border-gray-500/30 pb-6 overflow-hidden rounded-lg'>
      ...
      <div className='p-3 text-left'>
        ...
        <p className='text-gray-500'>Agnik</p>
        <div className='flex items-center space-x-2'>
          <p>{calculateRating(course)}</p>
          <div className='flex'> 
            {[...Array(5)].map((_, index) => (
              <img src={index < Math.floor(calculateRating(course)) ? assets.star : assets.star_blank} onClick={() => scrollTo(0, 0)} alt="" key={index} className='w-3.5 h3.5' />
            ))}
          </div>
