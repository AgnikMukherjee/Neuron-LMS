      ...
      <div className='flex md:flex-row flex-col-reverse gap-10 relative items-start justify-between md:px-36 px-8 md:pt-20 text-left'>
        ...
        <div className='max-w-xl z-10 text-gray-500 '>
          ...
          <div className='flex items-center space-x-2 pt-3 pb-1 text-sm'>
            ...
            <div className='flex'>
              {[...Array(5)].map((_, index) => (
                <img src={index < Math.floor(calculateRating(courseData)) ? assets.star : assets.star_blank} onClick={() => scrollTo(0, 0)} alt="" key={index} className='w-3.5 h3.5' />
              ))}
            </div>
          ...
          <p className='text-sm'>Course by <span className='text-blue-600 underline'>Agnik</span></p>
            ...
            <div className='pt-5'>
              {courseData.courseContent.map((chapter, index) => (
                <div key={index} className='border border-gray-300 bg-white mb-2 rounded'>
                  ...
                  <div className={`overflow-hidden transition-all duration-300 ease-in-out ${openSections[index] ? 'max-h-96' : 'h-0'}`}>
                    <ul className='list-disc md:pl-10 pl-4 pr4 py-2 text-gray-600 border-t borde-gray-300'>
                      {chapter.chapterContent.map((lecture, i) => (
                        <li key={i} className='flex items-start gap-2 py-1'>
                          <img src={assets.play_icon} alt="" className='w-4 h-4 mt-1' />
                          <div className='flex items-center justify-between w-full text-gray-800 text-xm md:text-default'>
                            <p>{lecture.lectureTitle}</p>
                            <div className='flex gap-2'>
                              {lecture.isPreviewFree && <p 
                                onClick={() => setPlayerData({videoId : lecture.lectureUrl.split('/').pop()})} className='text-blue-500 cursor-pointer'>Preview</p>}
        ...
        {/* right column */}
        <div className='max-w-course-card-sm md:max-w-course-card z-10 shadow-custom-card rounded-md overflow-hidden bg-white min-w-[300px] sm:min-w-[420px]'>
          {
                playerData ?  
          ...
          <div className='p-2 md:p-4'>
            ...
            <button className='md:mt-6 mt-4 w-full bg-blue-600 text-white py-3 rounded font-medium'
